...
from cta_classes.base_classes.data_reader_writer import EnvVarReader, Reader
...
class TwitterAPI_v2():
    ...
    def __init__(self) -> None:
        ...
        self.API_URL_MAP = Reader().read_resource(file_name=EnvVarReader().get_value("TWITTER_API_URL_MAP_FILE"))
...
class TwitterAPI(ApiAuthentication):
    ...
    def extract_following_list(self, user_id: Union[None, int] = None, base_user: bool = True, addidional_header_vals: dict = {}, additional_query_parameter_vals: dict = {}) -> dict:
        ...
        self.create_query_parameters(parameter_dict=DataTransformer().extend_dict_object(base_dict={"user.fields": "public_metrics"}, 
                                                                             additional_values=additional_query_parameter_vals))
    ...
    def get_tweets(self, user_id: str, start_search_time: Union[str, datetime] = None, stop_search_time: Union[str, datetime] = None, 
        ...
        # Prepare the api request
        self.create_header(
            header_dict=DataTransformer().extend_dict_object(base_dict={"User-Agent": "v2UserTweetsPython"}, additional_values=additional_header_vals)
        )
        self.create_query_parameters(
            parameter_dict=DataTransformer().extend_dict_object(base_dict={'end_time': dt_most_recent_string,  # bepaald de meest recente tweet die gelezen moet worden
                                                               'tweet.fields': 'created_at'},
                                                    additional_values=additional_query_parameter_vals)
        )
        self.create_url(user_id=user_id)
